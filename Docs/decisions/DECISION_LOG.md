...
## D-023 | 2026-05-30 | Simulation engine unified — `packages/simulation/` is canonical, studio local copy removed

- **Status:** Executed
- **Context:** `packages/simulation/` (3,835 lines) was a stale, never-imported copy of the engine. The live copy was `apps/studio/src/simulation/` (4,928 lines) with 127 `@/simulation/` import sites, 17 test files, and 3 additional files (dori, geometry, grid) that had a tertiary copy in `packages/core/src/simulation/`. This violated the no-duplicate-source-of-truth rule.
- **Decision:** Replace all 22 engine files in `packages/simulation/` with the studio's richer implementations (studio copy was larger, more developed, and what the app actually used). Add dori/geometry/grid to the package (importing from `@sentineltwin/core`). Move all 17 test files plus helpers/fixtures. Remove `apps/studio/src/simulation/` entirely. Update all 127 `@/simulation/` imports to `@sentineltwin/simulation`. Add path aliases to package tsconfig for workspace resolution. Keep package-only files (odpcvs.ts, critical-zone-selection.ts).
