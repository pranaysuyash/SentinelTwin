...
## D-304 | 2026-06-12 | Progressive-disclosure UI: calm defaults + floating selection task bar

**Decision:** The workspace adopts Adobe-style progressive disclosure as a design principle: the canvas at rest shows minimal glyphs; detail appears on intent (hover/selection); actions appear contextually near the work instead of living only in fixed chrome.

**Changes:**
1. **Calm defaults.** `overlayDensity` defaults to `compact` for all presets except debug (was `all`). Scene labels render as one-line chips at rest. The coverage legend and both preset pickers start collapsed — they are reference material, not primary chrome.
2. **Reveal on intent.** In compact density, hovering or selecting a camera promotes its chip to the full detail card; selecting a zone promotes its chip to the full status card. Hover off → back to the chip.
3. **Floating selection task bar** (`SelectionContextBar.tsx`). When an object is selected and the editor is idle, a small animated bar appears above the bottom canvas edge: node identity + the ≤5 highest-value contextual actions (Aim at Zone, Open Camera, Duplicate, Focus, Delete, etc.) + "Inspect" to open the right dock. It is built from the same `buildContextualMenuModel`/`planContextualAction` model as the right-click menu, and both surfaces now execute through a shared `applyContextActionPlan` executor — the two can never drift. Deeper actions remain in the right-click menu; full editing remains in the inspector. The bar hides while drawing, transforming, or aiming.

**Gate rationale:** the bar is gated on editor activity (`editorMode` idle/placing and not aiming), NOT on `activeTool === "select"`, because `selectNode` canonically retargets the active tool to the node's context.

**Verification (Tier 4):** Production build: density resolves to `compact`, legend collapsed, bar renders with curated camera actions, and Duplicate executed through the bar (5→6 cameras, "Duplicated camera" feedback, copy auto-selected). Screenshot evidence of the calm canvas + bar. Suite 929 pass / 0 fail.

## D-305 | 2026-06-12 | Analytics → Report convergence: reports embed the canonical analytics model

**Decision:** Report exports (`ReportLiteTab.buildHtmlReport`) embed the same `buildSecurityAnalyticsModel` output shown in the Analytics dashboard, rendered as inline-SVG charts via a new pure module `src/lib/report-analytics-export.ts` (`buildAnalyticsReportSection`, `ANALYTICS_REPORT_STYLES`). No parallel computation is introduced — the report's new "Security Analytics Overview" section (KPI cards, DORI distribution, issue-severity bars, coverage trend, camera leaderboard) is sourced from the identical headless model used by the dashboard.

**Why:** The dashboard (D-301) and report exports had begun to diverge — the report's "Advanced Risk Signals" table computed an overlapping but not-identical set of numbers ad hoc. Embedding the canonical model closes that gap for the metrics it covers and ensures the handoff artifact (the report) tells the same story as the interactive dashboard. The pre-existing "Advanced Risk Signals" table is retained as-is because it covers detail (fragility cell counts, k-robustness totals, placement-oracle coordinates, occlusion blame) not yet part of the analytics model's KPI set — both sections are additive, not duplicative of each other's source of truth.

**Verification (Tier 4):** New test `src/lib/__tests__/report-analytics-export.test.ts` (3 cases). `bun test` → 932/932 pass. `tsc --noEmit` clean. `pnpm --filter studio build` green.

## D-306 | 2026-06-12 | Coverage CI: snapshot-over-snapshot regression checks in the Analytics dashboard

**Decision:** A new pure module `src/lib/coverage-regression.ts` (`buildCoverageRegressionReport`) compares the live `SimulationResult` against the most recently saved simulated snapshot and reports CI-style pass/warn/fail checks for total coverage, blind area, critical-zone pass count, open issues (severity-weighted), and k-robustness. It reuses `buildZoneStatus`/`buildResilience` (newly exported from `security-analytics.ts`) rather than re-deriving zone/resilience interpretation — no parallel computation. Surfaced in `AnalyticsDashboardView.tsx` as a "Coverage CI" section above "Resilience", gated on a simulated baseline snapshot existing (`hasBaseline`); absent otherwise.

**Why:** This operationalizes the deep-analysis roadmap's comparative-analytics item — scene edits that regress coverage or break critical-zone passes should be visible immediately, the same way a CI pipeline flags a broken build, rather than requiring the user to mentally diff two dashboard views.

**Verification:** New test `src/lib/__tests__/coverage-regression.test.ts` (4 cases, 15 expects), all passing. `bun test` → 936/936 pass. `tsc --noEmit` clean. Browser-verified (Tier 4) for the empty-state gate (no baseline → card absent, no rendering errors). Populated-card screenshot verification deferred — see `Docs/todos/CURRENT_IMPLEMENTATION_STATE.md` sixth-pass addendum for the honest evidence-tier breakdown and follow-up.

**Deferred from this pass:** Item #2 (worker progress/cancellation) — `simulateStudio` runs synchronously with no yield points; restructuring it for progress reporting is a larger architectural change, re-scoped to its own pass rather than hacked in.

## D-307 | 2026-06-12 | Live Operational Health: simulated-vs-observed fusion in the Analytics dashboard

**Decision:** A new pure function `buildLiveOperationalHealth(scene, events)` in `security-analytics.ts` derives a live health snapshot for sensors and cameras from the operational evidence ledger (`sensor_heartbeat`/`sensor_faulted`/`sensor_restored`/`sensor_triggered` and `camera_live_connection_updated` events), tracking the most-recent event per node and classifying each as `healthy` | `degraded` | `fault`. No new computation source — it reads the same `OperationalEvidenceEvent[]` already powering the Evidence Ledger. Wired into `SecurityAnalyticsModel.liveHealth` and surfaced in `AnalyticsDashboardView.tsx` as a new "Live Operations" `SectionCard` (gated on `trackedSensors + trackedCameras > 0`, i.e. absent until live events exist).

**Why:** This is item #4 from `Docs/exploration/DEEP_ANALYSIS_BEST_IN_CLASS_2026-06-12.md` §4 — the dashboard currently only shows what the deterministic simulation *predicts*; this section shows what the live evidence ledger has actually *observed* (heartbeats, faults, camera connection status), giving the "Simulated vs. Observed" framing the architecture has been building toward (AI proposes, simulation verifies, AI explains — this adds the fourth leg: the field reports back).

**Verification (Tier 4):** New test `src/lib/__tests__/live-operational-health.test.ts` (4 cases, 15 expects): empty-but-honest summary with no events, sensor fault tracked and alerted, sensor restoration clears the alert, camera connection status classified by keyword. `bun test` → 940/940 pass (936 + 4 new). `tsc --noEmit` clean. Browser-verified end-to-end on a production build (`pnpm build` + `pnpm start`, since the preview tooling on this machine always runs `pnpm start` regardless of the requested command — `pnpm dev` requests are silently rerouted to `pnpm start`, so source changes require a manual rebuild before each verification pass): empty state confirmed (card absent with 0 live events), then a synthetic `sensor_faulted` event injected via `window.__sentinelStudioStore.setState(...)` produced a populated "Live Operations" card showing "1 sensor · 0 camera connections reporting · 1 need attention" and a FAULT-tagged `sensor_demo_1` / "Sensor fault reported" alert tile. Screenshot evidence captured.

## D-308 | 2026-06-12 | Camera Drift: design-time drift detection against the baseline snapshot

**Decision:** A new pure module `src/lib/camera-drift.ts` (`buildCameraDriftReport`) compares the live scene's `CameraNode[]` against the cameras captured in the most-recently-saved simulated snapshot (the same baseline Coverage CI / D-306 uses) and flags per-camera drift entries: `moved` (position delta vs. `POSITION_MINOR_M`/`POSITION_MAJOR_M` = 0.1m/0.5m), `reaimed` (yaw/pitch delta vs. `ANGLE_MINOR_DEG`/`ANGLE_MAJOR_DEG` = 2°/10°), `status_changed` (severity `major` if the new status is in `DEGRADED_STATUSES = {off, blocked, dirty, malfunctioning}` and the old one wasn't), `fov_changed` (FOV/range % delta vs. `FOV_MINOR_PCT`/`FOV_MAJOR_PCT` = 5%/20%), plus `added`/`removed` for camera-set changes. Wired into `AnalyticsDashboardView.tsx` as a "Camera Drift" `SectionCard` between "Coverage CI" and "Live Operations", gated on `cameraDriftReport && cameraDriftReport.entries.length > 0` — absent with no baseline or no drift.

**Why:** This is the design-time first slice of `Docs/exploration/EXPLORATION_MAP.md` §A1 ("live coverage drift detection"). A re-aimed, moved, or disabled camera silently invalidates a previously-verified coverage result — Camera Drift surfaces that immediately by diffing scene state the engine already has against the same baseline snapshot Coverage CI uses. No new schema, no new evidence types, no parallel truth (Rule 5 stays clean for this pass). The live-feed half (comparing against real camera telemetry, fusing with D-307's Live Operational Health) is a separate, larger architectural slice for a future pass.

**Verification (Tier 4):** New test `src/lib/__tests__/camera-drift.test.ts` (6 cases, 14 expects): no-baseline empty-but-honest report, no-drift-when-unchanged, major "moved" (>0.5m), major "reaimed" (>10°), major "status_changed" (`on → malfunctioning`), and `added`/`removed` camera-set diffing. `bun test` → 946/946 pass (940 + 6 new). `tsc --noEmit` clean. Browser-verified end-to-end on a production build (`pnpm build` + `pnpm start`): empty state confirmed (card absent with 0 snapshots); then added `cam_drift_test` via `window.__sentinelStudioStore.getState().addNode(...)`, ran `runSimulation()`, called `saveSnapshot("Baseline")`, then `updateNode("cam_drift_test", { position: [2,2,4.5], status: "malfunctioning" })` (1.5m move from `[2,2,3]`, `on → malfunctioning`). The "Camera Drift" card rendered with both entries as MAJOR: "Moved 1.50 m from baseline position" and "Status changed: on → malfunctioning". Screenshot evidence captured.

**Caught during implementation:** `CameraNode["position"]` is a tuple `[number, number, number]` (Zod `point3Schema = z.tuple([...])`), not `{x,y,z}` — an initial `.x/.y/.z` access in `distance()` produced `NaN` and silently zero drift entries. Caught via a `bun -e` debug script before it shipped as a false negative.

## D-309 | 2026-06-12 | Simulation worker progress reporting (item #2 of the deep-analysis roadmap)

**Decision:** The deterministic engine's async coverage path now reports incremental progress. `computeCoverageCellsAsync` in `packages/simulation/src/coverage.ts` accepts an optional `onProgress?: (fraction: number) => void` called every `yieldEvery` cells; `simulateStudioAsync` forwards it, mapping coverage-cell evaluation to `0..0.9` and reserving `0.9..1.0` for zone evaluation + result/recommendation assembly. `simulation.worker.ts` now runs `simulateStudioAsync` (previously the synchronous `simulateStudio`) and posts `{ id, type: "progress", fraction }` messages alongside the final `{ id, ok, ... }` response — a new variant of `SimulationRunResponse` in `simulation-run-core.ts`. `simulation-runner.ts` routes progress messages to a per-request `onProgress` callback (worker path) or passes it straight to `simulateStudioAsync` (main-thread fallback) via a new `SimulationRunOptions.onProgress`. The simulation slice exposes `simulationProgress: number | null` (null when idle), set to `0` at run start, updated on each progress callback (guarded against stale runs via the existing `scene.updatedAt` version check), and cleared to `null` on completion/error in all three terminal paths (`buildSimulationState`, the fix-sandbox success path, and both catch blocks). `TopBar.tsx`'s "Run Review" button and `SimStatus` chip now show a live percentage and the button renders a filling progress bar while reviewing.

**Why:** This is item #2 from `Docs/exploration/DEEP_ANALYSIS_BEST_IN_CLASS_2026-06-12.md` §4 ("Worker protocol → progress + cancellation. Request ids already exist; add progress messages so big scenes show a real progress bar"). `simulateStudioAsync` and its `yieldEvery` cooperative-yield loop already existed but were unused by the worker (which ran the synchronous `simulateStudio`) and had no progress hook. This pass wires the existing async/yielding path through to the UI without restructuring the engine or introducing a new execution model — `SimulationResult` and `SecurityScene` are unchanged (Rule 5 stays clean). Full request-cancellation (aborting an in-flight worker computation) remains out of scope — the existing stale-result-discard behavior (`scene.updatedAt` version check) already prevents a superseded run from overwriting newer state, which covers the most important case (the UI never shows results for an edit the user already moved past).

**Verification (Tier 4):** `bun test` → 946/946 pass (no count change — progress is additive, no new test file; existing coverage/simulate-studio/simulation-runner tests continue to exercise the unchanged result shape). `tsc --noEmit` clean. Browser-verified end-to-end on a production build (`pnpm build` + `pnpm start`): subscribed to the store and called `runSimulation()` — observed 28 incremental `simulationProgress` updates ramping `0 → 0.035 → 0.07 → ... → 0.9 → 1` before `simulationRunning` returned to `false` and `simulationProgress` returned to `null`, confirming the full worker → runner → store pipeline delivers live progress. The reference scene's simulation completes in tens of milliseconds, too fast to capture the in-progress UI state in a screenshot — itself confirms the worker path is fast for typical scenes; the store-level trace is the evidence of correct wiring.

**Revisit when:** larger scenes (or a future "big building" reference scene) make the progress bar visible long enough for a screenshot-level UI check, or when true mid-run cancellation (not just stale-result-discard) becomes a requirement — e.g. an "Abort review" button that calls `worker.terminate()` and rebuilds the worker.

## D-310 | 2026-06-12 | Cinematic framing grade: "shot quality" beyond DORI pixel density (EXPLORATION_MAP Thread 146)

**Decision:** A new pure module `src/lib/framing-grade.ts` (`buildFramingGradeReport`) derives, for every critical zone with at least one covering camera (status `on`), a per-camera "framing grade" — `well_framed` | `edge_of_frame` | `foreshortened` | `out_of_frame` — by projecting the zone's polygon centroid (at `scene.assumptions.personHeightM`) into the camera's frame using the *exact same* hAngle/vAngle projection `evaluatePoint` uses (`packages/simulation/src/coverage.ts` ~L573-579: `THREE.Vector3` direction → `atan2`-derived yaw/pitch → `normalizeAngle(targetYaw - camera.yawDeg)` / `targetPitch - camera.pitchDeg`). Each angle is normalized by `±fov/2` to give a `-1..1` in-frame position. Grading: `|norm| > 1` on either axis → `out_of_frame`; else `camera.pitchDeg <= -55°` (steep top-down) → `foreshortened`; else `|norm| > 0.6` on either axis → `edge_of_frame`; else `well_framed`. The report also lists `zonesNeedingAttention` — zones where no covering camera achieves `well_framed`. Wired into `AnalyticsDashboardView.tsx` as a new "Shot Quality" `SectionCard` (Clapperboard icon) between "Camera Drift" and "Live Operations", gated on `framingGradeReport.zonesNeedingAttention.length > 0` — absent when every covered zone is well-framed. Only non-`well_framed` entries render (the card surfaces problems, not a full matrix).

**Why:** This is Thread 146 from the 2026-06-12 director/simulator-lens exploration batch. DORI quality answers "is the pixel density high enough?" — it does not answer "where in the frame does this zone fall, and is the camera angle so steep it only ever sees the top of someone's head?" A zone can pass DORI while being framed at the extreme edge of the shot (lens-distortion risk, easy to walk out of frame) or shot from nearly overhead (foreshortened — "ID-tier" coverage that in practice shows a headtop, not a face). This is a pure derivation over data the engine already computes (camera pose + zone geometry) — no schema change, same Rule-5-clean shape as Camera Drift (D-308) and Coverage CI (D-306). Per the sequencing note in EXPLORATION_MAP.md, this was the lowest-lift, highest-differentiation item in the Thread 146-153 batch.

**Verification:** New test `src/lib/__tests__/framing-grade.test.ts` (4 cases, 12 expects): every covered zone/on-camera pair produces exactly one entry; every entry has a valid grade and in-frame positions are within `±1` unless `out_of_frame`; a camera aimed directly at a zone centroid is not `out_of_frame`; a camera at steep `-75°` pitch aimed at a zone centroid is graded `foreshortened`. `bun test` (run from `apps/studio`) → 950/950 pass (946 + 4 new). `tsc --noEmit` (from `apps/studio`) clean. `pnpm build` green.

**Revisit when:** the framing-grade vocabulary needs to feed into Thread 148 ("Director's Cut" cut-sequence selection — picking the camera with the best framing grade at time T) or Thread 150 (training-mode after-action review); or if zones need multiple sampled "action points" (e.g. doorway entry/exit points) instead of a single polygon centroid.

## D-311 | 2026-06-12 | Director's Cut: camera-cut sequence following the adversarial path (EXPLORATION_MAP Thread 148)

**Decision:** A new pure module `src/lib/directors-cut.ts` (`buildDirectorsCutSequence(scene, path)`) takes D-009's adversarial-path waypoints (the only per-time subject trajectory the engine produces today) and, for each waypoint, picks the camera with the best "shot" of the subject — the way a film editor cuts between cameras to follow a moving subject. `pickBestCamera` filters to cameras with `status === "on"` and `distance <= camera.rangeM`, grades each with D-310's `gradeCameraFraming(camera, point, targetHeightM)` (now exported from `framing-grade.ts` for reuse), excludes `out_of_frame`, and ranks the rest by `FRAMING_RANK` (`well_framed` < `edge_of_frame` < `foreshortened`), breaking ties by distance. A waypoint with no eligible camera gets `grade: "no_coverage"`. Adjacent waypoints with the same `cameraId`+`grade` collapse into a single cut `segment` with `startTimeS`/`endTimeS`/`startPosition`; `noCoverageDurationS` sums the duration of segments graded `no_coverage` or `out_of_frame`. Returns `null` only when the path has zero waypoints. Wired into `AnalyticsDashboardView.tsx` as a "Director's Cut" `SectionCard` (Clapperboard icon) right after "Trend & Activity" / Evidence Ledger, gated on `directorsCut && directorsCut.segments.length > 0`, showing the `% with no usable shot` headline and a row of cut-segment tiles (camera name, grade badge, time range).

**Why:** This is Thread 148 from the 2026-06-12 director/simulator-lens exploration batch, and the natural sequel to D-310: framing grade answers "how good is this shot?" for a static zone; Director's Cut applies the same grading along a moving subject's path to answer "which camera would actually be cutting to this person, second by second, and where does coverage go dark?" Three intended downstream uses per Thread 148: a sales-demo "trailer" (this is what your system would have caught), a forensic report companion paired with the adversarial path result, and guard-training material that visually shows "NO COVERAGE" cuts. Pure selection over existing engine outputs (camera pose/range/status + waypoint positions + D-310's projection) — no schema change, no new simulation math, Rule 5 stays clean.

**Verification (Tier 4):** New test `src/lib/__tests__/directors-cut.test.ts` (4 cases): `null` for zero-waypoint paths; one shot per waypoint with adjacent-same-camera collapsing into segments; segments cover the full path duration contiguously (`segments[i].startTimeS === segments[i-1].endTimeS`, first starts at `waypoints[0].timeS`, last ends at `totalDurationS`); all-cameras-off scene reports `no_coverage` for every shot with `noCoverageDurationS === totalDurationS`. `bun test` (from `apps/studio`) → 954/954 pass (950 + 4 new). `tsc --noEmit` (from `apps/studio`) clean. `pnpm build` green. Browser-verified on the production build (`scene_small_retail_shop`, fresh import): "Director's Cut" card rendered with headline "Camera-cut sequence following the adversarial path · 0% with no usable shot" and a single segment tile "Camera 3 — WELL FRAMED — 0.0s – 1.5s", matching the 6-waypoint adversarial path (all `exposedToCamera: "cam_entrance"`, `criticalZoneReachable: true`) all sharing one best camera/grade and collapsing to one segment. Screenshot evidence captured.

**Caught during investigation (not a bug):** an initial `document.body.innerText.includes("Director")` check returned `false` even after the card was correctly rendering — `innerText` reflects the CSS `text-transform: uppercase` applied to `SectionCard` titles, so the live DOM text is `"DIRECTOR'S CUT"`, not `"Director's Cut"`. The case-sensitive substring check was the false negative, not the component.

**Revisit when:** the engine produces additional subject trajectories beyond the single adversarial path (e.g. multiple intruder profiles, or the temporal-simulation (D-006/06_TEMPORAL_SIMULATION.md) staff/visitor flows from V0.3) — `buildDirectorsCutSequence` already takes `path` as a parameter, so extending to multiple sequences is additive; or when Thread 150 (training-mode after-action review) needs an actual video/timeline scrubber UI rather than a static segment list.

## D-312 | 2026-06-12 | Selection-overlay crowding fixes at narrow viewport widths

**Decision:** Pranay flagged that selecting a wall on a ~800px-wide canvas produced visibly overlapping UI: a redundant "Primary" floating label stacked on top of the "Move" transform-handle label, the floating selection task bar (D-304's `SelectionContextBar`) crowding the collapsed Guided-Edit/provider-status pill (`CommandBar`), and the bottom-left "Simulation Assumptions" card rendering label/value text literally on top of itself in its `grid-cols-2` layout. Three scoped fixes, all in `apps/studio/src/components/`:
1. `workspace/WorkspaceCanvas.tsx` `SelectionHighlights`: the per-node "Primary"/`+N` floating chip now renders only when `selectedNodeIds.length > 1`. For a single selection it was redundant (the selection task bar already names the object) and collided with the "Move" handle label at the same anchor point.
2. `command-bar/CommandBar.tsx` collapsed pill: the `mode.label`, `⌘K` hint, provider name, health, and budget-telemetry chips are now `hidden sm:inline` / `hidden md:inline` — on narrow viewports the pill shrinks to just the Guided-Edit sparkle icon, freeing the bottom-right corner the centered `SelectionContextBar` was colliding with.
3. `bottom-row/BottomRow.tsx` `AssumptionRow` + its container: container changed from `grid-cols-2` to `grid-cols-1` (the panel is already `overflow-y-auto`, so 8 stacked rows scroll instead of cramming into two columns that were narrower than their content); `AssumptionRow` itself gained `min-w-0`/`truncate` on the label so a long label can never overflow into the value column.

**Why:** This is a real usability bug, not cosmetic noise — overlapping, unreadable text in a security-analysis tool undermines trust in the "deterministic engine output" framing the rest of the dashboard works hard to establish. All three collisions share a root cause: multiple independently-positioned floating overlays (`absolute`/`SceneHtml`) and fixed-column grids that were tuned against a wide desktop canvas and never re-checked at the ~800px width a real laptop window (with the right inspector dock open) produces.

**Verification:** `bun test` (from `apps/studio`) → 954/954 pass (no count change — layout-only). `tsc --noEmit` clean. `pnpm build` green. Browser-verified at 800×830 viewport on the production build: selecting `wall_n` no longer shows a "Primary" chip, the Guided-Edit pill collapses to an icon-only button clear of the selection task bar, and the Simulation Assumptions card shows clean single-column rows (`OODPCVS 20`, `Normal`, `Simple`, `No windows`, `1.75 m`, ...) with no overlapping glyphs. Screenshot evidence captured before/after.

**Deferred (flagged by Pranay, larger scope):** "still so much... jargon and not actual industry/user speak" — labels like "OODPCVS 2025", "Night Penalty Mode", "Budget blocked", "K-Robustness", "DORI quality" are internal/engineering vocabulary. A pass to translate the dashboard/inspector copy into plain security-operator language (with the technical term as a secondary `ExplainBadge`/tooltip, not the primary label) is a real, valuable follow-up — tracked as a new EXPLORATION_MAP thread rather than folded into this layout fix, since it touches copy across the whole app and deserves its own pass with a copy audit.

**Revisit when:** doing the plain-language copy pass above — at that point, re-check whether the Simulation Assumptions card's 8-row single-column list is still the right density, or whether shortening labels (the actual fix for this deferred item) makes 2 columns viable again at narrow widths.

## D-313 | 2026-06-13 | Plain-language copy pass, first slice (EXPLORATION_MAP Thread 154)

**Decision:** First slice of Thread 154's dual-label pattern (plain-language primary label, technical/standard term as a smaller secondary label — not hidden, since security professionals may want to verify the standard at a glance):
1. `bottom-row/BottomRow.tsx` `AssumptionRow` gained an optional `sublabel` prop, rendered as `(sublabel)` in a smaller, dimmer span after the label, hidden below the `sm` breakpoint (`hidden sm:inline`) so it doesn't compete for space in the narrow "Simulation Assumptions" column. The "Simulation Assumptions" card's eight rows were renamed for plain operator language: "Quality Model" → "Image quality" (sublabel: "OODPCVS 2025"/"DORI 2014"), "Lighting Model" → "Indoor lighting", "Night Penalty" → "Night view" (sublabel: "Night penalty"), "Window Handling" → "Windows", "Person Height" → "Person height", "Time of Day" → "Time of day", "Wall Height" → "Wall height", "Night Mode" → "Night mode" (the editing-mode form's labels in `AssumptionsPanel.tsx` were intentionally left as-is for this slice).
2. `view/AnalyticsDashboardView.tsx` `SectionCard` gained an optional `subtitle` prop, rendered as a smaller, non-uppercase, dimmer `(subtitle)` next to the title. "DORI Quality Distribution" → "What can you actually see?" with subtitle "DORI quality distribution". In the Resilience section's K-Robustness tile, the tile heading became "Failure tolerance (k-robustness)" (the `K=N`/`—` value and existing plain-language description below were left unchanged).
3. `panels/ContextRightPanel.tsx`'s collapsed "Simulation Assumptions" summary line (shown when the section is collapsed, e.g. "IEC 62676-4:2025 · day · normal") was rewritten to "Latest image standard (IEC 62676-4:2025) · day · normal light" / "Legacy image standard (DORI 2014) · ...", keeping the technical term in parentheses per the dual-label pattern (and satisfying `context-right-panel.test.ts`'s existing `toContain("IEC 62676-4:2025")` assertion).

**Why:** Continues directly from D-312's deferred item — Pranay's feedback was that the dashboard/inspector is "still so much... jargon and not actual industry/user speak." This slice targets the two areas flagged in the thirteenth-pass addendum (Simulation Assumptions card + Resilience/DORI section headers) as a small, reviewable audit slice before expanding file-by-file. Copy-only change — no data model, schema, or simulation-engine change (Rule 5 stays clean).

**Verification (Tier 4):** `bun test` (from `apps/studio`) → 954/954 pass (no count change — copy-only). `tsc --noEmit` clean. `pnpm build` green. A grep for the renamed strings ("DORI Quality Distribution", "K-Robustness") confirmed they also appear in `src/lib/security-analytics.ts`, `pdf-export.ts`, `coverage-regression.ts`, `report-analytics-export.ts` and their tests — these are the report/export pipeline's own independently-generated HTML section headings (per D-305), separate from `AnalyticsDashboardView.tsx`'s `SectionCard` titles, and were correctly left untouched (`bun test` confirms no conflict). Browser-verified on the production build at both 1280×900 and 800×830 (re-confirming D-312's overlap fixes hold with the new copy):
- At 1280×900, the Analytics dashboard shows "WHAT CAN YOU ACTUALLY SEE? (DORI QUALITY DISTRIBUTION)" and the Resilience tile "FAILURE TOLERANCE (K-ROBUSTNESS)" wraps cleanly to two lines within its tile with `K=0` and the description rendering below without collision.
- At 800×830, selecting `wall_n` shows the "Simulation Assumptions" card with the new labels — "Image quality", "Indoor lighting", "Night view", "Windows", "Person height", "Time of day", "Wall height", "Night mode" — each truncating gracefully with an ellipsis in its ~102px column (no overlap; the `(sublabel)` is hidden at this width via `hidden sm:inline`). This is a single-line ellipsis truncation, not the overlapping/garbled text D-312 fixed.

**Note (caught during this slice):** the first attempt used longer labels ("Image quality standard", "Night-time estimate") with the sublabel always visible — at the ~102px-wide "Simulation Assumptions" column this truncated to unreadable fragments ("Ima...", "Ni..."). Fixed by shortening the labels themselves ("Image quality", "Night view") and hiding the sublabel below `sm`, rather than reverting to the old jargon labels.

**Revisit when:** continuing Thread 154 — remaining candidates from EXPLORATION_MAP Thread 154 are `AssumptionsPanel.tsx`'s editing-mode field labels, "Truth: Simulated", "Transmission" (glass material property), and "Budget blocked/guarded/ready" (CommandBar telemetry chips, appears in 6+ files including `WorkspaceCanvas.tsx`, `InspectorPanel.tsx`, `SceneBuilderWizard.tsx` — needs its own audit pass since it's load-bearing in several test files).

## D-314 | 2026-06-16 | Make every workspace package testable from the package directory by adding self-`paths`

**Decision:** All four workspace packages (`@sentineltwin/core`, `simulation`, `agents`, `report`) gain `tsconfig.json` `paths` entries that map their own package name to `./src/index.ts` (and `*` → `./src/*`), mirroring the existing `apps/studio/tsconfig.json` mappings. `packages/simulation/package.json` also gets explicit `devDependencies` for `suncalc`, `three`, `three-mesh-bvh`, and `@types/*` so its tests can resolve hoisted workspace deps via the package's own `node_modules` (pnpm's strict isolation does not auto-hoist into a package's own test directory).

**Root cause (verified):** Each package's `package.json` declares `main`/`exports` → `./dist/index.js`, but **no `dist/` is ever built**. The studio's `tsconfig.json` overrides this with explicit `paths` → source files, so `bun test` inside `apps/studio` resolves `@sentineltwin/simulation` to `packages/simulation/src/index.ts` directly (verified via `require.resolve` from inside the studio). But running `bun test packages/simulation/src/__tests__/*.test.ts` from the workspace root fails — `bun` does not pick up the studio's `paths` mapping, and the package's own `tsconfig.json` only has the `@sentineltwin/core` mapping, not self. Result: 20+ simulation tests are silently broken under any test runner that doesn't honor the studio's `tsconfig.json`. CI in `tools/studio-quality-gate.sh` only runs `bun test` from `apps/studio`, so the breakage was invisible from the gate.

**Rationale:**
- This blocks Tier 2+ verification for **any** engine change — a hard violation of motto §0.5 (Evidence Tiers), §14 (Validation Rules), and §19 (long-term coherence).
- The fix is the minimum necessary for root-cause closure (motto §7): point self-imports at the source (where tests should verify, not at the build artefact) and ensure hoisted deps are resolvable per-package. The studio's existing mappings prove this is the canonical pattern in this repo.
- Scope is "every workspace package" not just simulation, because the same `paths` gap will bite `agents`/`report` as soon as anyone tries to run those tests in isolation (or move them out of the studio for V0.3+).

**Alternatives rejected:**
- Build every package as a precondition for tests: rejected — adds CI time, creates build-before-test coupling, and tests should verify source not the build artefact.
- Move all simulation/agents/report tests into `apps/studio/src/**/__tests__/`: rejected — duplicates the canonical engine test location, breaks the package boundary, and creates the same parallel-truth violation motto §11 forbids.
- Add a workspace-root `bunfig.toml` test alias: rejected — `bunfig.toml` does not support import resolution aliases; only `tsconfig.json` `paths` work.
- Build only `simulation` (narrow fix): rejected — this is the exact "fix one instance, leave the pattern" trap motto §10 warns against.

**Verification:**
- `bun test packages/simulation/src/__tests__/seasonal-lighting.test.ts packages/simulation/src/__tests__/lighting.test.ts packages/simulation/src/__tests__/coverage.test.ts` from the workspace root now resolves and runs (Tier 2).
- `cd packages/simulation && bun test` runs the package's own test suite in isolation (Tier 2).
- `cd apps/studio && bun test` continues to show 954/954 pass (Tier 2, regression guard).
- `pnpm build` in `apps/studio` remains green (Tier 1, TypeScript still resolves through package `paths`).

**Revisit when:** the repo adopts a different test runner that doesn't honor package `tsconfig.json` `paths` (e.g. vitest with explicit aliases), or when packages are published externally and `dist/` becomes mandatory.

## D-315 | 2026-06-16 | Wire `computeSeasonalLightState()` into `getLightingContext()` for honest day/night coverage

**Decision:** `packages/simulation/src/coverage.ts`'s `getLightingContext()` now derives exterior `lightLevel` from `scene.timeSchedule` + the active simulation time via `computeSeasonalLightState()`. When `timeSchedule` is absent or has no `location`, the function falls back to the current behavior (`timeOfDay === "day" ? 1 : 0`). The simulation caller (`simulateStudio` / the studio's runner) supplies the current `(hour, minute)` through a new optional `CoverageContext.currentTime` field (typed `{ hour: number; minute: number }`), defaulting to `{ hour: 12, minute: 0 }` when not provided — preserving existing test fixtures that never set a time.

**Rationale:**
- The 24h temporal profile (`temporal.ts`) computes a full diurnal schedule including interior lights, occupancy, and exterior sun position, but the per-cell coverage evaluator ignored it. Heatmap, Camera View, and reports all render "day quality" for the same cells at 02:00 and 14:00 — a silent product-truth violation that contradicts the headline value prop ("eliminates false confidence about coverage").
- The seasonal module is already implemented, fully tested (`seasonal-lighting.test.ts` 18 cases pass), and correctly returns `null` when no location is set — so wiring it is a low-risk, additive change.
- The caller-driven time pattern preserves determinism (a core first principle of the engine per the docstring and `Docs/decisions/PRE_BUILD_DISCUSSION_LOG.md`) and avoids `new Date()` in the engine. Each call site controls which hour is simulated.
- The fallback path keeps every existing test (lighting.test.ts 4 cases, coverage.test.ts 6 cases) green because no fixture sets a `timeSchedule` and none pass a `currentTime`.

**Alternatives rejected:**
- Mutate `scene.assumptions.timeOfDay` from `computeSeasonalLightState`: rejected — overrides an explicit user-chosen assumption, breaks `scene-hash.ts` determinism, and would silently change every consumer that reads `timeOfDay`.
- Cache `SeasonalLightState` per scene: deferred — over-engineering until a profiler shows the call cost. `suncalc` is sub-millisecond per call.
- Per-cell sun occlusion: deferred — that's I14 territory (real CV adapter / Depth Anything V2 scale anchoring) and a much larger surface.
