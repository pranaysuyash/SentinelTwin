...
- The debug/provider-control surfaces now share canonical prompt definitions from a single registry file, the measured AI telemetry now persists prompt id/version/title/agent/schema lineage alongside each action, the prompt registry now also keeps a persisted history trail, the provider governance surface now keeps a persisted selection/policy history trail, and the AI telemetry trail now also compares recent runs against a longer-horizon policy baseline so the command parser, counterfactual agent, report agent, AI layout draft, and provider policy can be audited across time instead of only at the current snapshot ✅
