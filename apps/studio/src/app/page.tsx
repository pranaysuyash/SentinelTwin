...
import type { RuntimeIncidentInput } from "@/store/slices/telemetry-slice";
...
function StudioPageContent() {
  ...
  const simulationResultCurrent = simulationResult ?? scene.simulation ?? null;

  const [importError, setImportError] = useState<string | null>(null);
  const [isClientMounted, setIsClientMounted] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const bootstrapRef = useRef(false);
  const recordRuntimeIncidentRef = useRef<(payload: RuntimeIncidentInput) => void>(() => {});
  recordRuntimeIncidentRef.current = useStudioStore.getState().recordRuntimeIncident;

  useEffect(() => {
    setIsClientMounted(true);
  }, []);

  const recordRuntimeIncident = useCallback(
    (payload: RuntimeIncidentInput) => {
      recordRuntimeIncidentRef.current(payload);
    },
    [],
  );
  ...
  const launchWorkspace = (viewMode: "map" | "wall" | "replay" | "compare" | "report", preset: "edit" | "coverage" | "camera_wall" | "replay" | "compare" | "report" | "debug", bottomTab?: "metrics" | "issues" | "timeline" | "beforeafter" | "report" | "debug") => {
  ...
  const createDraftFromScene = (nextScene: SecurityScene, source: SceneIntakeSource, sourceArtifacts: string[] = []) => {
  ...
  useEffect(() => {
    ...
    if (simulationResultCurrent || !simulationDirty || scene.source !== "demo") return;
    runSimulationFromStore();
  }, [simulationResultCurrent, scene.source, runSimulationFromStore, simulationDirty]);
