...
export function ProviderConfigPanel({
  ...
  return (
    <div className={`{space-y-3 rounded-lg border UI_SURFACES.borderPanel UI_SURFACES.panel p-3}`}>
      {/* Header */}
      ...
      <div className="space-y-1">
        <span className={`text-[9px] font-medium UI_SURFACES.textSoftMid`}>Provider</span>
            ...
            <button type="button"
              ...
              className={`rounded-lg border px-2 py-1.5 text-left text-[9px] transition-colors ${
                selectedId === p.id
                  ...
                  : "UI_SURFACES.borderPanel UI_SURFACES.textSoftMid UI_SURFACES.hoverBorder UI_SURFACES.hoverTextSoft"
              }`}
            >
      ...
      {/* Model Selection */}
      <div className="space-y-1">
        <span className={`text-[9px] font-medium UI_SURFACES.textSoftMid`}>Model</span>
        <select
          value={selectedModel || currentProvider.defaultModel}
          onChange={(e) => handleModelChange(e.target.value)}
          className={`{w-full rounded-lg border UI_SURFACES.borderPanel UI_SURFACES.page px-2 py-1.5 text-[10px] UI_SURFACES.textBody outline-none focus:border-blue-500/40}`}
        >
      ...
      <button type="button"
        ...
        className={`text-[8px] font-medium UI_SURFACES.textDimMid UI_SURFACES.hoverTextSoft`}
      >
      ...
      {/* API Key Status */}
      {showAdvanced && (
        <div className={`{space-y-1.5 rounded-lg border UI_SURFACES.borderPanel UI_SURFACES.page p-2}`}>
          <span className={`text-[8px] font-medium uppercase tracking-wider UI_SURFACES.textDimMid`}>
            API Keys
          </span>
          {PROVIDERS.map((p) => {
            ...
            return (
              ...
              >
                <span className={`UI_SURFACES.textSoftMid`}>{p.envKey}</span>
                <span
                  className={`font-mono ${
          ...
          <div className={`pt-1 text-[7px] UI_SURFACES.textDim`}>
