...
export function PlacementPreviewPanel() {
  ...
  return (
    <div className="pointer-events-none absolute bottom-16 right-3 z-20 w-[260px] overflow-hidden rounded-xl border border-[#1f2536] bg-[#0b0f17]/96 shadow-2xl shadow-black/45">
      ...
      <div className="flex items-center justify-between gap-2 px-2.5 py-1.5 text-[9px] text-[#8b96b3]">
        <span>
          {pose.x.toFixed(1)}, {pose.z.toFixed(1)}m · {pose.yawDeg}° · FOV {pose.fovDeg}°
        </span>
        <span className="truncate text-[#6b7894]">{preset ? preset.label : "Default optics"}</span>
      </div>
    </div>
  );
}
