...
<body>
  ...
  using the ${escapeHtml(report.standardsRef)} standard.</p>
  ...
  <h2>Adversarial Path Analysis</h2>
  <p>Analysis of potential unauthorized approach paths.</p>
  <table>
  ...
  <h2>Compliance</h2>
  <p>
    <strong style="color:${report.codeCompliant ? "#16a34a" : "#dc2626"}">
      ${report.codeCompliant ? "✓ Compliant" : "✗ Non-compliant"}
    </strong>
    &mdash; ${report.summary.zonesPassing}/${report.summary.zonesTotal} zones meet required quality levels.
  </p>

  <footer>
    Generated by SentinelTwin Studio &middot; ${new Date(report.generatedAt).toISOString()} &middot;
    ${escapeHtml(report.standardsRef)}
  </footer>
</body>
</html>`;
}
...
export function exportAsMarkdown(report: ReportData): string {
  const lines = [
    ...
    ...(report.adversarialPath
      ...
      : []),
    "## Compliance",
    `**${report.codeCompliant ? "Compliant" : "Non-compliant"}** — ${report.summary.zonesPassing}/${report.summary.zonesTotal} zones meet requirements.`,
    "",
    `---`,
...
export function exportAsText(report: ReportData): string {
  const lines = [
    ...
    ...(report.recommendations.length > 0
      ...
      : []),
    "",
    `Compliance: ${report.codeCompliant ? "COMPLIANT" : "NON-COMPLIANT"}`,
