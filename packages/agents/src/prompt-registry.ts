import type { AiActionTelemetryStage } from "./types";

export type PromptRegistryStage = "command" | "counterfactual" | "report" | "draft" | "scene_understanding";
...
Output ONLY valid JSON matching the schema. Do not explain, do not add commentary.`,
  ...
  },
  {
    ...
    outputSchema: "SceneUnderstandingResult",
    systemPrompt: `You are a security assessment analyst. Analyze the provided scene description and simulation data to produce a structured security understanding.
...
Output ONLY valid JSON matching the schema. Frame findings defensively: "coverage gaps," "hardening recommendations," "authorized incident replay analysis." Never include "evasion," "bypass," or "defeat."`,
    note: "Canonical structured-output prompt for scene understanding and coverage gap analysis.",
  },
];
...
export function summarizePromptRegistry(registry: PromptRegistryEntry[] = PROMPT_REGISTRY): PromptRegistrySummary {
  return {
    ...
    stages: registry.reduce<Record<PromptRegistryStage, number>>((acc, entry) => {
      acc[entry.stage] += 1;
      return acc;
    }, { command: 0, counterfactual: 0, report: 0, draft: 0, scene_understanding: 0 }),
