...
function buildCausalTrace(report: ReportData): string[] {
  ...
  return [
    ...
    ...report.evidenceLedger.map((event) =>
      `- **${new Date(event.timestamp).toLocaleString()}** · ${event.title} · ${event.actor === "system" ? "System" : "User"}${event.confidence !== undefined && event.confidence !== 0 && event.confidence !== "withheld" ? ` (Confidence: ${event.confidence})` : ""}\n  - ${event.details}`
    ),
    "",
  ];
}
