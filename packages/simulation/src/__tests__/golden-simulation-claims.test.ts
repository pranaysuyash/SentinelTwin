...
import { simulateStudio, computeCoverageCells } from "@sentineltwin/simulation";
...
describe("golden simulation product claims", () => {
  ...
  test("redundancy is preserved with one camera offline", () => {
    ...
    const baselineZone = baseline.criticalZoneResults.find((z) => z.zoneId === "zone_redundant");
  ...
  test("moving an obstruction away improves critical zone outcome", () => {
  ...
  test("single camera failure preserves redundancy in a triple-camera setup", () => {
    ...
    expect(zone!.redundancyCameraCount).toBeGreaterThanOrEqual(2);
